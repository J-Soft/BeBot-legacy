...
class MySQL
{
    ...
    function connect($initial = false)
    {
        ...
        if (!$conn) {
            $this->error(
                "Cannot connect to the database server at " . $this->SERVER . " as user " . $this->USER . "!", $initial
            );
            return false;
        }
        if (!mysql_select_db($this->DBASE, $conn)) {
            $this->error(
                "Unable to select database " . $this->DBASE . " Database not found or insufficient priviledges!",
                $initial
            );
            return false;
        }
    ...
    */
    function get_tablename($table)
    {
        ...
        if (empty($name)) {
            ...
            $this->query(
                "INSERT INTO " . $this->master_tablename . " (internal_name, prefix, use_prefix) VALUES ('" . $table
                    . "', '" . $this->table_prefix . "', 'true')"
            );
        }
        else {
    ...
    */
    ...
    function set_version($table, $version)
    {
        if (!is_numeric($version)) {
            ...
            $this->query(
                "UPDATE " . $this->master_tablename . " SET schemaversion = " . $version . " WHERE internal_name = '"
                    . $table . "'"
            );
            $usep = $this->select(
