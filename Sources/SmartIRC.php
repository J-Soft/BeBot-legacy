...
class Net_SmartIRC_base
{
    ...
    function setUseSockets($boolean)
    {
        if ($boolean === true) {
            if (@extension_loaded('sockets')) {
                ...
                $this->log(
                    SMARTIRC_DEBUG_NOTICE, 'WARNING: socket extension not loaded, trying to load it...', __FILE__,
                    __LINE__
                );
                ...
                if ($load_status) {
                    $this->log(
                        SMARTIRC_DEBUG_NOTICE, 'WARNING: socket extension succesfully loaded', __FILE__, __LINE__
                    );
                    $this->_usesockets = true;
                }
                else {
                    $this->log(
                        SMARTIRC_DEBUG_NOTICE, 'WARNING: couldn\'t load the socket extension', __FILE__, __LINE__
                    );
                    $this->log(
                        SMARTIRC_DEBUG_NOTICE,
                        'WARNING: your PHP build doesn\'t support real sockets, will use fsocks instead', __FILE__,
                        __LINE__
                    );
    ...
    function setChannelSynching($boolean)
    {
        $this->log(
            SMARTIRC_DEBUG_NOTICE,
            'WARNING: you are using setChannelSynching() which is a deprecated method, use setChannelSyncing() instead!',
            __FILE__, __LINE__
        );
    ...
    function setChannelSyncing($boolean)
    {
        ...
        if ($this->_channelsyncing == true) {
            $this->log(
                SMARTIRC_DEBUG_CHANNELSYNCING, 'DEBUG_CHANNELSYNCING: Channel syncing enabled', __FILE__, __LINE__
            );
        }
        else {
            $this->log(
                SMARTIRC_DEBUG_CHANNELSYNCING, 'DEBUG_CHANNELSYNCING: Channel syncing disabled', __FILE__, __LINE__
            );
        }
    }
    ...
    function setLogdestination($type)
    {
        ...
        default:
            $this->log(
                SMARTIRC_DEBUG_NOTICE, 'WARNING: unknown logdestination type (' . $type . '), will use STDOUT instead',
                __FILE__, __LINE__
            );
    ...
    function showBenchmark()
    {
        $this->log(
            SMARTIRC_DEBUG_NOTICE,
            'benchmark time: ' . ((float)$this->_benchmark_stoptime - (float)$this->_benchmark_starttime), __FILE__,
            __LINE__
        );
    }
    ...
    function log($level, $entry, $file = null, $line = null)
    {
        ...
        if (!(is_integer($level)) || !($level & SMARTIRC_DEBUG_ALL)
            ...
            $this->log(
                SMARTIRC_DEBUG_NOTICE, 'WARNING: invalid log level passed to log() (' . $level . ')', __FILE__, __LINE__
            );
            return;
        }
    ...
    function connect($address, $port)
    {
        ...
        if ($result === false) {
            ...
            if ($this->_usesockets != true) {
                $this->_socket = $result;
                $this->log(
                    SMARTIRC_DEBUG_SOCKET, 'DEBUG_SOCKET: activating nonblocking fsocket mode', __FILE__, __LINE__
                );
    ...
    function disconnect($quickdisconnect = false)
    {
        ...
        if ($this->_channelsyncing == true) {
            ...
            $this->log(
                SMARTIRC_DEBUG_CHANNELSYNCING, 'DEBUG_CHANNELSYNCING: cleaned channel array', __FILE__, __LINE__
            );
        }
    ...
    )
    {
        ...
        if (!is_numeric($usermode)) {
            $this->log(
                SMARTIRC_DEBUG_NOTICE,
                'DEBUG_NOTICE: login() usermode (' . $usermode . ') is not valid, will use 0 instead', __FILE__,
                __LINE__
            );
            $usermode = 0;
        }
        ...
        $this->_send(
            'USER ' . $this->_username . ' ' . $usermode . ' ' . SMARTIRC_UNUSED . ' :' . $this->_realname,
            SMARTIRC_CRITICAL
        );
    }
    ...
    function isJoined($channel, $nickname = null)
    {
        if ($this->_channelsyncing != true) {
            $this->log(
                SMARTIRC_DEBUG_NOTICE,
                'WARNING: isJoined() is called and the required Channel Syncing is not activated!', __FILE__, __LINE__
            );
            return false;
        }
    ...
    function isOpped($channel, $nickname = null)
    {
        if ($this->_channelsyncing != true) {
            $this->log(
                SMARTIRC_DEBUG_NOTICE,
                'WARNING: isOpped() is called and the required Channel Syncing is not activated!', __FILE__, __LINE__
            );
            return false;
        }
    ...
    function isVoiced($channel, $nickname = null)
    {
        if ($this->_channelsyncing != true) {
            $this->log(
                SMARTIRC_DEBUG_NOTICE,
                'WARNING: isVoiced() is called and the required Channel Syncing is not activated!', __FILE__, __LINE__
            );
            return false;
        }
    ...
    function isBanned($channel, $hostmask)
    {
        if ($this->_channelsyncing != true) {
            $this->log(
                SMARTIRC_DEBUG_NOTICE,
                'WARNING: isBanned() is called and the required Channel Syncing is not activated!', __FILE__, __LINE__
            );
            return false;
        }
    ...
    )
    {
        ...
        if (!$this->_isValidType($handlertype)) {
            $this->log(
                SMARTIRC_DEBUG_NOTICE, 'WARNING: passed invalid handlertype to registerActionhandler()', __FILE__,
                __LINE__
            );
            return false;
        }
        ...
        $this->log(
            SMARTIRC_DEBUG_ACTIONHANDLER, 'DEBUG_ACTIONHANDLER: actionhandler(' . $id . ') registered', __FILE__,
            __LINE__
        );
        return $id;
    }
    ...
    )
    {
        ...
        if (!$this->_isValidType($handlertype)) {
            $this->log(
                SMARTIRC_DEBUG_NOTICE, 'WARNING: passed invalid handlertype to unregisterActionhandler()', __FILE__,
                __LINE__
            );
            return false;
        }
        ...
        for ($i = 0; $i < $handlercount; $i++) {
            ...
            if ($handlerobject->type == $handlertype && $handlerobject->message == $regexhandler
                ...
                $this->log(
                    SMARTIRC_DEBUG_ACTIONHANDLER, 'DEBUG_ACTIONHANDLER: actionhandler(' . $id . ') unregistered',
                    __FILE__, __LINE__
                );
        ...
        $this->log(
            SMARTIRC_DEBUG_ACTIONHANDLER,
            'DEBUG_ACTIONHANDLER: could not find actionhandler type: "' . $handlertype . '" message: "' . $regexhandler
                . '" method: "' . $methodname . '" from object "' . get_class($object) . '" _not_ unregistered',
            __FILE__, __LINE__
        );
        return false;
    }
    ...
    function unregisterActionid($id)
    {
        ...
        $this->log(
            SMARTIRC_DEBUG_ACTIONHANDLER,
            'DEBUG_ACTIONHANDLER: could not find actionhandler id: ' . $id . ' _not_ unregistered', __FILE__, __LINE__
        );
        return false;
    }
    ...
    function registerTimehandler($interval, &$object, $methodname)
    {
        ...
        $this->log(
            SMARTIRC_DEBUG_TIMEHANDLER, 'DEBUG_TIMEHANDLER: timehandler(' . $id . ') registered', __FILE__, __LINE__
        );
    ...
    function unregisterTimeid($id)
    {
        ...
        for ($i = 0; $i < $handlercount; $i++) {
            ...
            if ($handlerobject->id == $id) {
                ...
                $this->log(
                    SMARTIRC_DEBUG_TIMEHANDLER, 'DEBUG_TIMEHANDLER: timehandler(' . $id . ') unregistered', __FILE__,
                    __LINE__
                );
        ...
        $this->log(
            SMARTIRC_DEBUG_TIMEHANDLER,
            'DEBUG_TIMEHANDLER: could not find timehandler id: ' . $id . ' _not_ unregistered', __FILE__, __LINE__
        );
        return false;
    }
    ...
    function _send($data, $priority = SMARTIRC_MEDIUM)
    {
        ...
        default:
            $this->log(
                SMARTIRC_DEBUG_NOTICE, 'WARNING: message (' . $data . ') with an invalid priority passed (' . $priority
                . '), message is ignored!', __FILE__, __LINE__
            );
            return false;
        }
    }
        ...
        for ($i = 0; $i < count($this->_timehandler); $i++) {
            ...
            if ($microtimestamp >= ($handlerobject->lastmicrotimestamp + ($handlerobject->interval / 1000))) {
                ...
                if (@method_exists($methodobject, $method)) {
                    $this->log(
                        SMARTIRC_DEBUG_TIMEHANDLER,
                        'DEBUG_TIMEHANDLER: calling method "' . get_class($methodobject) . '->' . $method . '"',
                        __FILE__, __LINE__
                    );
    ...
    function _checktimeout()
    {
        if ($this->_autoreconnect == true) {
            ...
            if ($this->_lastrx < ($timestamp - $this->_rxtimeout)) {
                $this->log(
                    SMARTIRC_DEBUG_CONNECTION, 'DEBUG_CONNECTION: receive timeout detected, doing reconnect...',
                    __FILE__, __LINE__
                );
                $this->reconnect();
            }
                ...
                if ($this->_lasttx < ($timestamp - $this->_txtimeout)) {
                    $this->log(
                        SMARTIRC_DEBUG_CONNECTION, 'DEBUG_CONNECTION: transmit timeout detected, doing reconnect...',
                        __FILE__, __LINE__
                    );
                    $this->reconnect();
                }
    ...
    function _rawreceive()
    {
        ...
        if ($this->_state() == SMARTIRC_STATE_CONNECTED) {
            ...
            if ($this->_usesockets == true) {
                ...
                if ($result == 1) {
                    ...
                    if ($result === false) {
                        ...
                        $this->log(
                            SMARTIRC_DEBUG_NOTICE,
                            'WARNING: socket_select() returned false, something went wrong! Reason: ' . socket_strerror(
                                socket_last_error()
                            ), __FILE__, __LINE__
                        );
                        exit;
            ...
            while (count($rawdataar) > 0) {
                ...
                $this->log(
                    SMARTIRC_DEBUG_IRCMESSAGES, 'DEBUG_IRCMESSAGES: received: "' . $rawline . '"', __FILE__, __LINE__
                );
    ...
    function _gettype($line)
    {
        if (preg_match('/^:[^ ]+? [0-9]{3} .+$/', $line) == 1) {
            ...
            default:
                $this->log(
                    SMARTIRC_DEBUG_IRCMESSAGES, 'DEBUG_IRCMESSAGES: replycode UNKNOWN (' . $code . '): "' . $line . '"',
                    __FILE__, __LINE__
                );
            }
        }
                                                ...
                                                if (preg_match('/^:.*? PART .*$/', $line) == 1) {
                                                    ...
                                                    if (preg_match('/^:.*? MODE .* .*$/', $line) == 1) {
                                                        ...
                                                        if (preg_match('/^:.*? QUIT :.*$/', $line) == 1) {
                                                            ...
                                                            $this->log(
                                                                SMARTIRC_DEBUG_MESSAGETYPES,
                                                                'DEBUG_MESSAGETYPES: SMARTIRC_TYPE_UNKNOWN!: "' . $line
                                                                    . '"',
                                                                __FILE__,
                                                                __LINE__
                                                            );
    ...
    function _updatestate()
    {
        ...
        if ((is_resource($this->_socket)) && ($this->_socket !== false)
    ...
    function _handlemessage($messagecode, &$ircdata)
    {
        ...
        if (is_numeric($messagecode)) {
            if (!array_key_exists($messagecode, $this->nreplycodes)) {
                $this->log(
                    SMARTIRC_DEBUG_MESSAGEHANDLER,
                    'DEBUG_MESSAGEHANDLER: ignoring unreconzied messagecode! "' . $messagecode . '"', __FILE__, __LINE__
                );
                $this->log(
                    SMARTIRC_DEBUG_MESSAGEHANDLER,
                    'DEBUG_MESSAGEHANDLER: this IRC server (' . $this->_address . ') doesn\'t conform to the RFC 2812!',
                    __FILE__, __LINE__
                );
                return;
            }
        ...
        if (@method_exists($this, $_methodname)) {
            $this->log(
                SMARTIRC_DEBUG_MESSAGEHANDLER,
                'DEBUG_MESSAGEHANDLER: calling internal method "' . get_class($this) . '->' . $_methodname . '" ('
        ...
        if (@method_exists($this, $methodname)) {
            $this->log(
                SMARTIRC_DEBUG_MESSAGEHANDLER,
                'DEBUG_MESSAGEHANDLER: calling user defined method "' . get_class($this) . '->' . $methodname . '" ('
                    . $_codetype . ')', __FILE__, __LINE__
            );
        ...
        if ($found == false) {
            $this->log(
                SMARTIRC_DEBUG_MESSAGEHANDLER,
                'DEBUG_MESSAGEHANDLER: no method found for "' . $messagecode . '" (' . $methodname . ')', __FILE__,
                __LINE__
            );
        }
    }
    ...
    function _handleactionhandler(&$ircdata)
    {
        ...
        for ($i = 0; $i < $handlercount; $i++) {
            ...
            if (($handlerobject->type & $ircdata->type) && (preg_match($regex, $ircdata->message) == 1)
                ...
                $this->log(
                    SMARTIRC_DEBUG_ACTIONHANDLER,
                    'DEBUG_ACTIONHANDLER: actionhandler match found for id: ' . $i . ' type: ' . $ircdata->type
                        . ' message: "' . $ircdata->message . '" regex: "' . $regex . '"', __FILE__, __LINE__
                );
                ...
                if (@method_exists($methodobject, $method)) {
                    $this->log(
                        SMARTIRC_DEBUG_ACTIONHANDLER,
                        'DEBUG_ACTIONHANDLER: calling method "' . get_class($methodobject) . '->' . $method . '"',
                        __FILE__, __LINE__
                    );
                    ...
                    $this->log(
                        SMARTIRC_DEBUG_ACTIONHANDLER,
                        'DEBUG_ACTIONHANDLER: method doesn\'t exist! "' . get_class($methodobject) . '->' . $method
                            . '"', __FILE__, __LINE__
                    );
                }
    ...
    function _adduser(&$channel, &$newuser)
    {
        ...
        if (isset($channel->users[$lowerednick])) {
            $this->log(
                SMARTIRC_DEBUG_CHANNELSYNCING,
                'DEBUG_CHANNELSYNCING: updating user: ' . $newuser->nick . ' on channel: ' . $channel->name, __FILE__,
                __LINE__
            );
            ...
            $this->log(
                SMARTIRC_DEBUG_CHANNELSYNCING,
                'DEBUG_CHANNELSYNCING: adding user: ' . $newuser->nick . ' to channel: ' . $channel->name, __FILE__,
                __LINE__
            );
        ...
        if ($user->op) {
            $this->log(
                SMARTIRC_DEBUG_CHANNELSYNCING,
                'DEBUG_CHANNELSYNCING: adding op: ' . $user->nick . ' to channel: ' . $channel->name, __FILE__, __LINE__
            );
        ...
        if ($user->voice) {
            $this->log(
                SMARTIRC_DEBUG_CHANNELSYNCING,
                'DEBUG_CHANNELSYNCING: adding voice: ' . $user->nick . ' to channel: ' . $channel->name, __FILE__,
                __LINE__
            );
    ...
    function _removeuser(&$ircdata)
    {
        if ($ircdata->type & (SMARTIRC_TYPE_PART | SMARTIRC_TYPE_QUIT)) {
            ...
            if ($ircdata->type & SMARTIRC_TYPE_KICK) {
                ...
                $this->log(
                    SMARTIRC_DEBUG_CHANNELSYNCING,
                    'DEBUG_CHANNELSYNCING: unknown TYPE (' . $ircdata->type . ') in _removeuser(), trying default',
                    __FILE__, __LINE__
                );
                $nick = $ircdata->nick;
            }
        ...
        if ($this->_nick == $nick) {
            $this->log(
                SMARTIRC_DEBUG_CHANNELSYNCING,
                'DEBUG_CHANNELSYNCING: we left channel: ' . $ircdata->channel . ' destroying...', __FILE__, __LINE__
            );
            ...
            if ($ircdata->type & SMARTIRC_TYPE_QUIT) {
                $this->log(
                    SMARTIRC_DEBUG_CHANNELSYNCING,
                    'DEBUG_CHANNELSYNCING: user ' . $nick . ' quit, removing him from all channels', __FILE__, __LINE__
                );
                ...
                foreach ($this->_channels as $channelkey => $channelvalue) {
                    ...
                    foreach ($channel->users as $userkey => $uservalue) {
                        ...
                        if ($nick == $uservalue->nick) {
                            ...
                            $this->log(
                                SMARTIRC_DEBUG_CHANNELSYNCING,
                                'DEBUG_CHANNELSYNCING: found him on channel: ' . $channel->name . ' destroying...',
                                __FILE__, __LINE__
                            );
                            ...
                            if (isset($channel->ops[$nick])) {
                                // die!
                                $this->log(
                                    SMARTIRC_DEBUG_CHANNELSYNCING, 'DEBUG_CHANNELSYNCING: removing him from op list',
                                    __FILE__, __LINE__
                                );
                            ...
                            if (isset($channel->voices[$nick])) {
                                // die!!
                                $this->log(
                                    SMARTIRC_DEBUG_CHANNELSYNCING, 'DEBUG_CHANNELSYNCING: removing him from voice list',
                                    __FILE__, __LINE__
                                );
                ...
                $this->log(
                    SMARTIRC_DEBUG_CHANNELSYNCING,
                    'DEBUG_CHANNELSYNCING: removing user: ' . $nick . ' from channel: ' . $ircdata->channel, __FILE__,
                    __LINE__
                );
                ...
                if (isset($channel->ops[$nick])) {
                    $this->log(
                        SMARTIRC_DEBUG_CHANNELSYNCING, 'DEBUG_CHANNELSYNCING: removing him from op list', __FILE__,
                        __LINE__
                    );
                    unset($channel->ops[$nick]);
                }
                ...
                if (isset($channel->voices[$nick])) {
                    $this->log(
                        SMARTIRC_DEBUG_CHANNELSYNCING, 'DEBUG_CHANNELSYNCING: removing him from voice list', __FILE__,
                        __LINE__
                    );
                    unset($channel->voices[$nick]);
                }
