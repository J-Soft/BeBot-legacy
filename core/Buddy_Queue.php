    ...
    */
    function __construct(&$bot)
    {
        ...
        $this->bot->core("settings")
            ->create(
            "Buddy_Queue", "Enabled", TRUE,
            "Should buddies be queued or added as requested? (Queueing buddies may slow down the bot.)"
        );
        $this->bot->core("settings")
            ->create(
            "Buddy_Queue", "Rate", 1, "How many buddy add and removes should be done per second?",
    ...
    function do_add($uid)
    {
        if (!empty($uid) && $uid != 0 && $uid != -1) {
            ...
            $this->bot->log(
                ...
                ->core("chat")
                ->get_uname($uid) . " as a buddy when they already are one."
            );
        }
    }
        ...
        if (!empty($uid) && $uid != 0 && $uid != -1) {
            if (($this->bot->core("chat")->buddy_exists($uid))) {
                ...
                $this->bot->log(
                    ...
                    ->core("chat")
                    ->get_uname($uid) . " as a buddy when they are not one."
                );
            }
        }
    }
