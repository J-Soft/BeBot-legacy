...
*  USA
*
* File last changed at $LastChangedDate: 2008-08-02 22:25:41 +0100 (Sat, 02 Aug 2008) $
* Revision: $Id: BotStatistics.php 3 2008-08-02 21:25:41Z temar $
*/
...
class BotStatistics_Core extends BasePassiveModule
{
    ...
    function start()
    {
        ...
        if (empty($result)) {
            ...
            ) // Make sure Bot was Online long enough to do time stamp with cron to prevent Spamming into log if crashloop.
            {
                $this->bot->db->query(
                    "INSERT INTO " . $this->DB . "#___bots_log (bot, dim, start, end) VALUES ('" . $result[0][0]
    ...
    function check_bots($name, $origin, $bot = FALSE, $dim = FALSE)
    {
        ...
        if ($bot) {
            ...
            $result = $this->bot->db->select(
                "SELECT bot, dim, online, time, start, total, restarts FROM " . $this->DB . "#___bots WHERE bot = '"
                    . $bot . "' AND dim = '" . $dim . "'"
            );
            if (!empty($result)) {
                $bot = $result[0];
                ...
                $log = $this->bot->db->select(
                    "SELECT start, end FROM " . $this->DB . "#___bots_log WHERE bot = '" . $bot[0] . "' AND dim = '"
                        . $bot[1] . "'"
                );
                $day = 60 * 60 * 24;
                $daytime = time() - $day;
            ...
            $result = $this->bot->db->select(
                "SELECT bot, dim, online, time FROM " . $this->DB . "#___bots ORDER BY dim, online DESC"
            );
            if (!empty($result)) {
