...
* Revision: $Id: 09_AccessControl.php 1833 2008-11-30 22:09:06Z alreadythere $
*/
...
class AccessControl_Core extends BasePassiveModule
{
    ...
    function __construct(&$bot)
    {
        ...
        */
        ...
        $this->bot->core("settings")
            ->create(
            "AccessControl", "LockGc", FALSE, "Are all commands in the guild chat locked from use?", "On;Off", TRUE
        );
        $this->bot->core("settings")
            ->create(
            "AccessControl", "LockPgroup", FALSE, "Are all commands in the private chatgroup locked from use?",
            "On;Off", TRUE
        );
    ...
    function update_table()
    {
        if ($this->bot->core("settings")
            ...
            $this->bot->db->set_version(
                "access_control",
                $this->bot->core("settings")
                    ->get("accesscontrol", "schemaversion")
            );
                ...
                if (!(empty($this->bot->commands[$channel[$i]]))) {
                    ...
                    foreach ($this->bot->commands[$channel[$i]] as $key => $val) {
                        $rights = $this->bot->db->select(
                            "SELECT * FROM #___access_control WHERE command = '"
                                . $key . "' AND channel = '" . $channel[$i] . "'"
                        );

                        if (empty($rights)) {
                            $this->bot->db->query(
                                "INSERT INTO #___access_control (command, subcommand, channel, "
    ...
    function do_check($user, $command, $subcommand, $channel)
    {
        ...
        if (in_array(
            $this->access_cache[strtolower($command)][strtolower($subcommand)][strtolower($channel)], $this->deny_levels
        )
        ) {
            return false;
        }
            ...
            return $this->bot->core("security")->check_access(
                $user,
    ...
    function update_access($command, $channel, $newlevel)
    {
        ...
        $this->bot->db->query(
            "INSERT INTO #___access_control (command, subcommand, channel, minlevel) VALUES ('" . $command
    ...
    function create($channel, $command, $defaultlevel)
    {
        ...
        if (!(in_array($defaultlevel, $this->access_levels)) || $defaultlevel == 'DELETED') {
            $this->bot->log(
                "ACCESS", "ERROR", "Trying to set an illegal default level of " . $defaultlevel
        ...
        if (!(in_array($channel, $this->channels))) {
            $this->bot->log(
                "ACCESS", "ERROR", "Trying to set default access rights for an illegal channel: "
                . $channel . " for " . $command . "!"
            );
            return;
        }
        ...
        if ($channel == "all") {
            ...
            foreach ($chans as $chan) {
                ...
                $this->bot->db->query(
                    "INSERT IGNORE INTO #___access_control (command, subcommand, channel, minlevel) VALUES ('"
            ...
            // Add the default level if no entry for the channel/command combination exists yet:
            $this->bot->db->query(
                "INSERT IGNORE INTO #___access_control (command, subcommand, channel, minlevel) VALUES ('" . $command
    ...
    function update($command, $sub, $channel, $newlevel)
    {
        ...
        if (!(in_array($newlevel, $this->access_levels))) {
            $this->bot->log(
                "ACCESS", "ERROR", "Trying to set an illegal default level of " . $newlevel
        ...
        if ($newlevel == 'DELETED' && $sub == '*') {
            $this->bot->log(
                "ACCESS", "ERROR", "You cannot delete commands (subcommand * entries)! "
        ...
        $this->bot->db->query(
            "INSERT INTO #___access_control (command, subcommand, channel, minlevel) VALUES ('" . $command
                . "', '" . $sub . "', '" . $channel . "', '" . $newlevel . "') ON DUPLICATE KEY UPDATE minlevel = '"
                . $newlevel . "'"
        );
    ...
    function create_subcommand($channel, $command, $sub, $defaultlevel)
    {
        ...
        if ($sub == "*") {
            $this->bot->log(
                "ACCESS", "ERROR", "You cannot set access levels for the general subcommand * "
    ...
    function save($name, $desc, $update = FALSE)
    {
        ...
        foreach ($this->access_cache as $command => $value) {
            ...
            foreach ($value as $subcom => $value2) {
                ...
                if (!empty($chans)) {
                    if ($chans["tell"] && $chans["tell"] == $chans["gc"] && $chans["tell"] == $chans["pgmsg"]
                        && !$chans["extpgmsg"]
                    ) {
        ...
        if ($update) {
            $sql
                = " ON DUPLICATE KEY UPDATE description = '" . mysql_real_escape_string($desc) . "', commands = '" . $save
                . "'";
        }
        $this->bot->db->query(
            "INSERT INTO #___access_control_saves (name, description, commands) VALUES ('" . mysql_real_escape_string(
                $name
            ) . "', '" . mysql_real_escape_string($desc) . "', '" . $save . "')" . $sql
        );
        Return (array(
            $count,
            $countsub
    ...
    function load($name)
    {
        ...
        $results = $this->bot->db->select(
            "SELECT commands FROM #___access_control_saves WHERE name = '" . mysql_real_escape_string($name) . "'"
        );
        if (!empty($results)) {
            ...
            foreach ($load as $coms) {
                if (isset($this->bot->commands[$coms[2]][$coms[0]])) {
                    $this->bot->db->query(
                        "UPDATE #___access_control SET minlevel = '" . $coms[3] . "' WHERE command = '" . $coms[0]
                            . "' AND subcommand = '" . $coms[1] . "' AND channel = '" . $coms[2] . "'"
                    );
