...
class Timer_Core extends BasePassiveModule
{
    ...
    function __construct(&$bot)
    {
        ...
        $this->bot->core("settings")
            ->create(
            "Timer", "DefaultClass", "Standard",
            "Which is the name of the default class to use for timers if no other is specified?"
        );
        $this->bot->core("settings")
            ->create(
            "Timer", "GuestChannel", "origin",
            "Should an timer started in guestchannel or org-chat show up in both those channels or just in the channel of origin?",
            "both;origin"
        );
        $this->bot->core("settings")
            ->create(
        ...
        $this->bot->core("settings")
            ->create(
            "Timer", "DeleteRank", "ADMIN", "Minimal access level to remove timer of other players.",
        ...
        $this->bot->core("settings")
            ...
            "What's the minimal repeat interval for repeating timers (in seconds)? This is used to prevent annoying spam.",
            "5;10;15;20;25;30;45;60"
        );
        $this->bot->core("settings")
            ->create(
            "Timer", "RecoveryCheckInterval", 10,
            "How many minutes should pass between each check to recover after a database failures?", "5;10;15;30;45;60"
        );
        ...
        $classid = $this->create_timer_class(
            "LowSpam", "Default timer class with less notifies then the Standard class."
        );
        ...
        $classid = $this->create_timer_class(
            "None",
            "Default timer class containing no notifies at all, not even any added output when the timer runs out."
        );
    ...
    function update_timer_table()
    {
        ...
        case 1:
            ...
            $this->bot->db->update_table(
                "timer_class_entries", "notify_prefix", "add",
            ...
            if (empty($res)) {
                $this->bot->db->query(
                    "UPDATE #___timer_class_entries SET notify_prefix = 'Timer' "
                        . "WHERE notify_prefix = ''"
                );
            }
        }
        ...
        if (!empty($timers)) {
            foreach ($timers as $timer) {
                    ...
                    if (isset($this->modules[strtolower($timer['owner'])])) {
                        if ($this->modules[strtolower($timer['owner'])] != NULL) {
                            ...
                            $this->modules[strtolower($timer['owner'])]->timer(
                                $timer['name'], $this->class_cache[$timerclass]['prefix'],
                ...
                if ($timer['endtime'] > $thistime) {
                    $this->bot->db->query(
                        "UPDATE #___timer SET timerclass = "
                ...
                elseif ($timer['repeatinterval'] > 0) {
                    ...
                    if ($this->class_cache[$timerclass]['class'] == "Default") {
                        ...
                        $entry = $this->get_class_entry(
                            $this->class_cache[$timerclass]['class'], $timer['repeatinterval']
                        );
                    }

                    // Now simply update the existing timer in the table
                    $this->bot->db->query(
                        "UPDATE #___timer SET timerclass = " . $entry
    ...
    function get_class_entry($class, $duration)
    {
        $classret = $this->bot->db->select(
            "SELECT t1.id FROM #___timer_class_entries AS t1, #___timer_classes AS t2"
    ...
    )
    {
        ...
        $this->bot->db->query(
            "INSERT INTO #___timer (name, timerclass, endtime, owner, channel, repeatinterval) VALUES "
        ...
        if (!$relaying && $channel != "tell" && $channel != 'internal'
            && $this->bot
                ->core("settings")->get("timer", "relay")
            && $this->bot->core("relay") instanceof Relay
            && $this->bot
                ->core("settings")->get('Relay', 'Status')
        ) {
            $msg
                = "<pre>relaytimer class:" . $class . " endtime:" . $endtime . " owner:" . $owner . " repeat:" . $repeat;
    ...
    function list_timed_events($owner)
    {
        return $this->bot->db->select(
            "SELECT id, name FROM #___timer WHERE channel = 'internal' AND owner = '"
                . $owner . "'", MYSQL_ASSOC
        );
    }
    ...
    {
        return $this->bot->db->select(
            "SELECT id, name, timerclass, channel, owner, endtime, repeatinterval "
    ...
    function create_class_cache()
    {
        ...
        $classentries = $this->bot->db->select(
            "SELECT id, class_id, next_id, notify_delay, notify_prefix, notify_suffix FROM #___timer_class_entries",
            MYSQL_ASSOC
        );
        ...
        $settings_entries = $this->bot->db->select(
            "SELECT id, name, current_class FROM #___timer_class_settings", MYSQL_ASSOC
        );
    ...
    {
        $this->bot->db->query(
            "INSERT IGNORE INTO #___timer_classes (name, description) VALUES ('"
                . mysql_real_escape_string($name) . "', '" . mysql_real_escape_string($description) . "')"
        );

        $id = $this->bot->db->select(
            "SELECT id FROM #___timer_classes WHERE name = '" . mysql_real_escape_string($name) . "'"
        );
        if (empty($id)) {
            return -1;
        }
    ...
    {
        $this->bot->db->query(
            "INSERT IGNORE INTO #___timer_class_entries (next_id, class_id, notify_delay, notify_prefix, "
                . "notify_suffix) VALUES (" . $next_id . ", " . $class_id . ", " . $delay . ", '"
                . mysql_real_escape_string($prefix) . "', '"
                . mysql_real_escape_string($suffix) . "')"
        );

        $id = $this->bot->db->select(
            "SELECT id FROM #___timer_class_entries WHERE class_id = " . $class_id
        ...
        if ($delay == 0 || $next_id == -2) {
            $this->bot->db->query(
                "UPDATE #___timer_class_entries SET next_id = " . $id[0][0] . " WHERE class_id = "
    ...
    {
        $id = $this->bot->db->select(
            "SELECT id FROM #___timer_classes WHERE name = '"
                . mysql_real_escape_string($name) . "'", MYSQL_ASSOC
        );
        ...
        if (empty($id)) {
            $id = $this->bot->db->select(
                "SELECT id FROM #___timer_classes WHERE name = '"
                    ...
                    $this->bot
                        ->core("settings")->get("Timer", "DefaultClass")
                )
                    . "'", MYSQL_ASSOC
            );
        }
    ...
    function create_class_setting($name, $default_class_name, $description)
    {
        ...
        $class_setting = $this->bot->db->select(
            "SELECT id, current_class FROM #___timer_class_settings WHERE name = '"
                . mysql_real_escape_string($name) . "'", MYSQL_ASSOC
        );
        ...
        if (empty($class_setting)) {
            ...
            $this->bot->db->query(
                "INSERT IGNORE INTO #___timer_class_settings (name, current_class, "
                    . "default_class, description) VALUES ('" . mysql_real_escape_string($name) . "', '"
            ...
            // UPDATE entry
            $this->bot->db->query(
                "UPDATE #___timer_class_settings SET default_class = " . $default_id
                    . ", description = '" . mysql_real_escape_string($description) . "' WHERE id = "
                    . $class_setting[0]['id']
            );
        }
    }
    ...
    {
        $class_setting = $this->bot->db->select(
            "SELECT id FROM #___timer_class_settings WHERE name = '"
                . mysql_real_escape_string($name) . "'", MYSQL_ASSOC
        );
        ...
        if (!empty($class_setting)) {
            ...
            $this->bot->db->query(
                "UPDATE #___timer_class_settings SET current_class = "
                    . $class_id . " WHERE id = " . $class_setting[0]['id']
            );
    ...
    function get_class_setting($name)
    {
        ...
        $cname = $this->bot->db->select(
            "SELECT name FROM #___timer_classes WHERE id = "
                . $this->settings_cache[strtolower($name)], MYSQL_ASSOC
        );
        if (empty($cname)) {
            return "";
        }
