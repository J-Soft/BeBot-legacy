...
class CommandAlias_Core extends BasePassiveModule
{
    ...
    function unregister($alias)
    {
        ...
        $get = $this->bot->db->select(
            "SELECT alias, command FROM #___command_alias WHERE alias = '" . mysql_real_escape_string($alias) . "'"
        );
    ...
    function add($msg)
    {
        ...
        if (!isset($this->alias[$var[0]])) {
            if ($var[0] !== "comalias") {
                $this->bot->db->query(
                    "INSERT INTO #___command_alias (alias, command) VALUES ('" . mysql_real_escape_string($var[0])
                        . "', '" . mysql_real_escape_string($var[1]) . "')"
                );
                $this->alias[$var[0]] = $var[1];
                Return ("##highlight##" . $var[0] . "##end## is now an alias of ##highlight##" . $this->alias[$var[0]]
                    . "##end##!");
            }
            else {
            ...
            Return ("##highlight##" . $var[0] . "##end## is already an alias of ##highlight##" . $this->alias[$var[0]]
                . "##end##!");
        }
    }
    ...
    function del($alias)
    {
        ...
        if (!empty($get)) {
            $this->bot->db->query(
                "DELETE FROM #___command_alias WHERE alias = '" . mysql_real_escape_string($alias) . "'"
            );
            unset($this->alias[$alias]);
