...
*  USA
*
* File last changed at $LastChangedDate: 2008-09-02 01:37:45 +0100 (Tue, 02 Sep 2008) $
* Revision: $Id: 05_Maintenance.php 72 2008-09-02 00:37:45Z temar $
*/
    ...
    */
    ...
    function connect()
    {
        ...
        $this->bot->core("settings")
            ->create(
            "Maintenance", "info", "", "Info saved while restarting, blank when not doing maintenance.", NULL, TRUE, 2
        );
    ...
    function step2($name, $mode, $origin)
    {
        ...
        Switch ($mode) {
        case 'settings':
            $olddata = $this->bot->db->select(
                "SELECT module, setting, datatype, longdesc, defaultoptions, hidden, disporder FROM #___settings"
            );
            if (!empty($olddata)) {
                ...
                if (!isset($new[strtolower($mod)][strtolower($set)])) {
                    ...
                    if ($compare) {
                        foreach ($compare as $id => $name) {
                            if (!isset($this->dontupdate[$mod][$set][$name])
                                && $value[$id] != $new[strtolower($mod)][strtolower($set)][$id]
                            ) {
                                $this->update[] = array(
                                    $mod,
                                    $set,
        ...
        if (!empty($this->update)) {
            foreach ($this->update as $u) {
                $inside
                    ...
                    ->core("tools")
                    ->chatcmd(
                    "maintenance settings dont update " . $u[0] . " " . $u[1] . " " . $u[2], "[Dont Change]"
                ) . "\n";
            }
        }
        $inside .= "\n ::: " . $this->bot->core("tools")
            ...
            if ($info != "") {
                ...
                if (!empty($this->del)) {
                    foreach ($this->del as $del) {
                        $this->bot->db->query(
                            "DELETE FROM #___settings WHERE module = '" . $del[0] . "' AND setting = '" . $del[1] . "'"
                        );
                    }
                }
                if (!empty($this->update)) {
                    foreach ($this->update as $up) {
                        $this->bot->db->query(
                            "UPDATE #___settings SET " . $up[2] . " = '" . $up[4] . "' WHERE module = '" . $up[0]
                                . "' AND setting = '" . $up[1] . "'"
                        );
                    }
                }
