    ...
    function show_log($category = false)
    {
        if ($category === false) {
            ...
            $inside .= $this->bot->core("tools")
                           ->chatcmd("log all", "Show the last " . $this->bot
                ->core("settings")
        ...
        elseif ($category == "all")
        {
            $results = $this->bot->db->select("SELECT first,second,message,timestamp FROM #___log_message ORDER BY timestamp DESC LIMIT " . $this->bot
                ->core("settings")->get("Log", "LimitMessage"));

            if (!empty($results)) {
                foreach ($results AS $result)
                {
                    $inside .= "[" . gmdate($this->bot->core("settings")
        ...
        {
            $results = $this->bot->db->select("SELECT first,second,message,timestamp FROM #___log_message WHERE first = '" . $category . "' ORDER BY timestamp DESC LIMIT " . $this->bot
                ->core("settings")->get("Log", "LimitMessage"));
