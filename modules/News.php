...
class News extends BaseActiveModule
{
    ...
    */
    function get_news($name)
    {
        ...
        if (!empty($result_headline)) {
            ...
            {
                $inside .= "##ao_infoheader##On " . gmdate($this->bot
                    ->core("settings")
                    ->get("Time", "FormatString"), $val[1]) . " GMT ##ao_cctext##" . $val[2] . "##end## Reported:\n";
                $inside .= "##ao_infotext##" . stripslashes($val[3]);
                if (($this->bot->core("security")
                    ->check_access($name, $this->bot->core("settings")
                    ->get('News', 'Headline_del'))) || ($name == $val[2])
                ) {
        ...
        if (!empty($result)) {
            $newsdate = gmdate($this->bot->core("settings")
    ...
    */
    function get_raids($name)
    {
        ...
        if (!empty($result_raids)) {
            ...
            {
                $inside .= "##ao_infoheader##" . gmdate($this->bot
                    ->core("settings")
                    ->get("Time", "FormatString"), $val[1]) . " GMT ##ao_cctext##" . $val[2] . "##end## wrote:\n";
                $inside .= " ##ao_infotext##" . stripslashes($val[3]);
    ...
    */
    ...
    function del_news($name, $msg)
    {
        ...
        if (($this->bot->core("security")->check_access($name, $this->bot
            ->core("settings")
            ->get('News', 'News_del'))) || ($name == $res_name)
        ) {
        ...
        {
            $this->error->set("You must be " . $this->bot->core("settings")
