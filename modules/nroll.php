...
class Nroll Extends BaseActiveModule
{
    ...
    function __construct(&$bot)
    {
        ...
        $this->help['command']['nroll keywords']
            = "Randomly choose one of several keywords, seperated by commas or if no commas are present, by spaces.";
    ...
    function command_handler($name, $msg, $origin)
    {
        ...
        elseif (preg_match("/^nverify (.+)$/i", $msg, $info)) {
            if (isset($this->verifyresult[$info[1]])) {
                ...
                $output
                    = "Results not found.  Please check your query and try again.  If that doesn't work, give up, it ain't worth it.";
            }
            //$this -> bot -> send_output($name, $output, $origin);
        }
        return $output;
