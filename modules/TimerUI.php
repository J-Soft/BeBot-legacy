...
class TimerGUI extends BaseActiveModule
{
  ...
  {
    $tsets = $this->bot->db->select('SELECT a.id AS id, a.name AS name, b.name AS class, a.description as description '
  ...
  {
    $tsets = $this->bot->db->select('SELECT a.id AS id, a.name AS name, b.name AS current_class, '
  ...
  function update_timer_setting($tsetid, $newclassid)
  {
    ...
    if ($this->bot->core("timer")->update_class_setting($tset['name'], $class['name'])) {
      return "Changed timer class setting ##highlight##" . $tset['name'] . "##end## to new timer class ##highlight##"
        . $class['name'] . "##end##";
    }
