...
class TimerGUI extends BaseActiveModule
{
    function __construct(&$bot)
    {
        ...
        $this->bot->core("timer")
            ...
            "PublicTimer", "LowSpam", "The default class used for timers in a public channel (org, pgroup)."
        );
        $this->bot->core("timer")
    ...
    function command_handler($name, $msg, $channel)
    {
        ...
        case 'rtimer':
            ...
            elseif (preg_match(
                "/^rtimer ([a-z]+ )?([0-9]+(:[0-9][0-9]){0,3}) ([0-9]+(:[0-9][0-9]){0,3}) (.*)/i", $msg, $info
            )
            ) {
                if ($info[1] != "") {
                    $classused = $info[1];
    ...
    function show_timer($name, $channel)
    {
        ...
        $timers = $this->bot->db->select(
            "SELECT * FROM #___timer WHERE " . $channelstr . $namestr . " ORDER BY endtime ASC", MYSQL_ASSOC
        );
        if (empty($timers)) {
    ...
    {
        $tsets = $this->bot->db->select(
            'SELECT a.id AS id, a.name AS name, b.name AS current_class, '
                . 'c.name AS default_class, c.id AS default_id, a.description as description '
                . 'FROM #___timer_class_settings AS a, #___timer_classes AS b, #___timer_classes AS c '
                . 'WHERE a.current_class = b.id AND a.default_class = c.id AND a.id = ' . $tsetid,
            MYSQL_ASSOC
        );
    ...
    function update_timer_setting($tsetid, $newclassid)
    {
        ...
        if ($this->bot->core("timer")
            ...
            return
                "Changed timer class setting ##highlight##" . $tset['name'] . "##end## to new timer class ##highlight##"
                . $class['name'] . "##end##";
        }
