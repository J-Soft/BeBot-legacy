...
* Revision: $Id: OnlineCount.php 1833 2008-11-30 22:09:06Z alreadythere $
*/
...
class OnlineCounting extends BaseActiveModule
{
    function __construct(&$bot)
    {
        ...
        $this->help['command']['count all']
            = "Lists all professions and the number of characters of each " . $this->cp . " in chat";
        $this->help['command']['count'] = $this->help['command']['count all'];
        $this->help['command']['count [prof]']
            = "Lists all members of [prof] with level and alien level that are in chat.";
        $this->help['command']['count org']
            = "Lists the number of characters per organization currently online in chat.";
        $this->help['command']['count org [orgname]']
            = "Lists the number of characters online in chat that are in the organization [orgname].";
        ...
        $this->help['command']['check org [orgname]']
            = "Offers assist on all characters online in chat that are in the organization [orgname].";
    }
    ...
    function get_org_members($orgname)
    {
        return $this->bot->db->select(
            "SELECT DISTINCT(t1.nickname), t2.level, t2.defender_rank_id"
                . " FROM " . $this->bot->core("online")
                ->full_tablename()
    ...
    function get_prof($prof)
    {
        ...
        return $this->bot->db->select(
            "SELECT DISTINCT(t1.nickname), t2.level, t2.defender_rank_id"
                . " FROM " . $this->bot->core("online")
                ->full_tablename()
                . " WHERE t2." . $this->cp . " " . $profsearch . " ORDER BY t1.nickname ASC"
        );
    }
    ...
    function get_orgs()
    {
        ...
        $sql = "SELECT t1.org AS org, t1.count AS count, SUM(t2.level) / t1.count AS avg_level FROM ("
            ...
            . "t2.nickname = t3.nickname AND " . $this->bot->core("online")
            ->otherbots("t3.")
    ...
    function count_all()
    {
        ...
        $query = "SELECT t2." . $this->cp . " as profession, COUNT(DISTINCT t1.nickname) as count"
            . " FROM " . $this->bot->core("online")->full_tablename()
    ...
    function count($shortcut)
    {
        ...
        $pcount = $this->bot->db->select(
            "SELECT COUNT(DISTINCT t1.nickname) FROM "
    ...
    function count_org()
    {
        ...
        $tcount = $this->bot->db->select(
            "SELECT count(DISTINCT nickname) as count FROM #___online WHERE "
                . $this->bot->core("online")
                ->otherbots("") . " AND "
                . $this->bot->core("online")
                ->channels(""), MYSQL_ASSOC
        );
        ...
        return $this->bot->core("tools")
            ->make_blob(
            "Online organizations", "##blob_title##Online organisations:##end##<br><br>" . implode("<br>", $orgs)
        );
    }
