    ...
    function get_blacklist($name)
    {
        ...
        if (!empty($result)) {
            foreach ($result as $val) {
                ...
                $inside .= "##blob_text##" . $val['name'] . " - Set by: ##red##" . $note['author'] . "##end## Reason: "
                    . $note['note'] . " Expires: " . $expire_string . "##end##\n";
            }
        }
    ...
    function table_update()
    {
        ...
        case 0: // Previous version of BeBot.
            $this->bot->db->update_table(
                "blacklist", "noteid", "add", "ALTER IGNORE TABLE #___blacklist ADD noteid INT NOT NULL"
            );
            $this->bot->db->update_table(
                "blacklist", "expire", "add", "ALTER IGNORE TABLE #___blacklist ADD expire INT UNSIGNED DEFAULT 0"
            );
