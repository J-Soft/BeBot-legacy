...
class Rolls extends BaseActiveModule
{
    function __construct(&$bot)
    {
        ...
        $this->bot->core("settings")
            ->create(
            "Loot", "Roll", "SINGLE", "Should you be allowed to be added to the roll of more than one slot?",
            "SINGLE;MULTI"
        );
        ...
        $this->help['command']['loot add <slot>']
            = "Adds your name to the slot number.  Add 0 removes you from all slots.";
    ...
    */
    ...
    { // Start function handler()
        ...
        case 'reroll':
            ...
            break;
        case 'rem':
            if (is_numeric($com['args']) && (int)$com['args'] == (real)$com['args']
                && array_key_exists(
                    $name, $this->loot[$com['args']]
                )
            ) {
                unset($this->loot[$com['args']][$name]);
                $this->bot->send_gc(
                    "##loot_highlight##" . $name . "##end## removed from rolls in slot##loot_highlight## #"
                        . $com['args']
                );
            }
            else {
        ...
        if ($slot == 0) {
            ...
            if ($this->loot[$slot]) {
                if ($this->bot->core("settings")
                    ...
                    if ($present == true) {
                        $this->addmsg
                            = "##loot_highlight##" . $name . "##end## changed to slot##loot_highlight## #" . $slot
                            . "##end##";
                    }
                    else {
                        $this->addmsg
                            = "##loot_highlight##" . $name . "##end## assigned to slot##loot_highlight## #" . $slot
                            . "##end##";
                    }
                    ...
                    $this->loot[$slot][$name] = 1;
                    $this->addmsg = "##loot_highlight##" . $name . "##end## assiged to slot##loot_highlight## #" . $slot
                        . "##end##";
                }
            }
            else {
    ...
    function reroll($name)
    {
        ...
        if ($lcount == 0) {
            ...
            foreach ($this->leftovers as $item) {
                ...
                $msg .= "##loot_highlight##" . $num . "x " . $item . "##end## being rolled in slot##loot_highlight## #"
                    . $numslot . "##end##.\n";
            }
    ...
    function rlist()
    {
        ...
        foreach ($this->loot as $slot) {
            ...
            $msg
                .= "Item: ##loot_highlight##" . $slot[item] . "##end## (##loot_highlight##" . $slot[num] . "x##end##)\n";
