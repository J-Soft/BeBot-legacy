...
class Points extends BaseActiveModule
{
    ...
    */
    function __construct(&$bot)
    {
        ...
        $this->help['command']['points [name]']
            = "Shows the amount of points in [name]s account. If [name] is not given it shows the points in your account";
        $this->help['command']['points give <name> <points>'] = "Gives <points> points to player <name>";
        $this->help['command']['points add <name> <points> <why>']
            = "Adds <points> points to player <name>s point account";
        $this->help['command']['points [del/rem] <name> <points> <why>']
        ...
        $this->help['command']['points tomain <(on|off)>']
            = "Turns ability to give points from alts to main on or off.";
    ...
    function show_points($name, $target)
    {
        if (!$target || strtolower($target) == strtolower($name)) {
            $result = $this->bot->db->select(
                "SELECT points, nickname FROM #___raid_points WHERE id = " . $this->points_to($name)
            );
            if ($result) {
                if ($result[0][1] == "") {
                    $this->bot->db->query(
                        "UPDATE #___raid_points SET nickname = '" . $this->points_to_name($name) . "' WHERE id = "
                            . $this->points_to($name)
                    );
                }
            ...
            if ($this->bot->core("security")->check_access($name, "admin")) {
                if (!$this->bot->core("chat")->get_uid($target)) {
                    ...
                    $result = $this->bot->db->select(
                        "SELECT points, nickname FROM #___raid_points WHERE id = " . $this->points_to($target)
                    );
                    if ($result) {
                        if ($result[0][1] == "") {
                            $this->bot->db->query(
                                "UPDATE #___raid_points SET nickname = '" . $this->points_to_name($target)
                                    . "' WHERE id = " . $this->points_to($target)
                            );
                        }
                        $points = $this->round($result[0][0]);
                    }
    ...
    */
    function all_points($name)
    {
        ...
        $result = $this->bot->db->select(
            "SELECT nickname, points FROM #___raid_points WHERE points > 0 ORDER BY points DESC"
        );
    ...
    function top_points($name)
    {
        ...
        if (!empty($result)) {
            ...
            foreach ($result as $val) {
                ...
                $val[1] = $this->round($val[1]);
                $inside
                    .= $num . ". ##highlight##" . $val[0] . "##end##$space - ##highlight##" . ($val[1]) . "##end##\n";
                $num++;
            }
            $this->bot->send_tell(
    ...
    */
    function tomain_points($name, $toggle)
    {
        ...
        if ($check) {
            ...
            $this->bot->send_tell(
                $name, "Points going to the main character's account is now ##highlight##" . $txt . "##end##." . $add
            );
        }
        //}
        //else
    ...
    function check_alts($main, $name = FALSE)
    {
        ...
        if (!empty($alts)) {
            foreach ($alts as $alt) {
                $result = $this->bot->db->select(
                    "SELECT id, nickname, points FROM #___raid_points WHERE points != 0 AND id = " . $this->points_to(
                        $alt, FALSE
                    )
                );
                if (!empty($result)) {
                    ...
                    if ($res[0] != $this->points_to($res[1])) {
                        $resu = $this->bot->db->select(
                            "SELECT nickname, points FROM #___raid_points WHERE id = " . $this->points_to($res[1])
                        );
                        if (empty($resu)) {
                            $this->bot->db->query(
                                "INSERT INTO #___raid_points (id, nickname, points, raiding) VALUES ("
                                    . $this->points_to($res[1]) . ", '" . $this->points_to_name($res[1]) . "', "
                                    . $res[2] . ", 0)"
                            );
                            $resu = $this->bot->db->select(
                                "SELECT nickname, points FROM #___raid_points WHERE id = " . $this->points_to($res[1])
                            ...
                            $this->bot->db->query(
                                "UPDATE #___raid_points SET points = " . ($res[2] + $resu[0][1]) . " WHERE id = "
                                    . $this->points_to($res[1])
                            );
                        }
                        ...
                        if (!empty($check) && (($check[0][1] * 100) != (($res[2] * 100) + ($resu[0][1] * 100)))) {
                            $this->bot->log(
                                "Points", "Error",
                                "Error With Transfering Points from Alt $alt (" . $res[2] . ") to $main (" . $resu[0][1]
                                    . ") Check = " . $check[0][1], TRUE
                            );
                        }
                        else {
                            $this->bot->db->query("UPDATE #___raid_points SET points = 0 WHERE id = " . $res[0]);
                            $string
                                = $res[1] . ": " . $this->round($res[2]) . " + " . $resu[0][0] . ": " . $this->round(
                                $resu[0][1]
                            ) . " = " . $resu[0][0] . ": " . $this->round($res[2] + $resu[0][1]);
                            $this->bot->log("Points", "Transfer", $string, TRUE);
                        }
                    }
                }
                $result = $this->bot->db->select(
                    "SELECT id FROM #___raid_points WHERE raiding = 1 and id = " . $this->points_to($alt, FALSE)
                );
                if (!empty($result)) {
                    $res = $result[0];
                    $this->bot->db->query("UPDATE #___raid_points SET raiding = 0 WHERE id = " . $res[0]);
                    $this->bot->db->query(
                        "UPDATE #___raid_points SET raiding = 1 WHERE id = " . $this->points_to($alt)
                    );
                }
            }
        }
    ...
    */
    function transfer_points($name, $toggle)
    {
        if ($this->bot->core("security")->check_access($name, "superadmin")) {
            ...
            $this->bot->send_tell(
                $name, "Transfering points has been ##highlight##" .
                $txt . "##end##."
            );
        }
        else {
        ...
        if ($this->bot->core("settings")->get("Points", "Transfer")) {
            ...
            if ($num > ($result[0][0])) {
                ...
                if (!$this->bot->core("chat")->get_uid($who)) {
                    ...
                    $this->bot->db->query(
                        "UPDATE #___raid_points SET points = points - " . ($num) .
                            " WHERE id = " . $this->points_to($name)
                    );
                    $this->bot->db->query(
                        "INSERT INTO #___raid_points (id, nickname, points) VALUES (" . $this->points_to($who) . ", '"
                            . $this->points_to_name($who)
                            . "', $num) ON DUPLICATE KEY UPDATE points = points + VALUES(points)"
                    );
                    $this->bot->send_tell(
                        $name, "You gave ##highlight##$num##end## raidpoints to ##highlight##$who##end##."
                    );
                    $this->bot->send_tell(
                        $who, "You got ##highlight##$num##end## raidpoints from ##highlight##$name##end##."
                    );
                    return;
                }
            }
    ...
    */
    function add_points($name, $who, $num, $why, $silent = FALSE)
    {
        ...
        if (!$this->bot->core("chat")->get_uid($who)) {
            ...
            $this->bot->db->query(
                "INSERT INTO #___raid_points (id, nickname, points) VALUES (" . $this->points_to($who) . ", '"
                    . $this->points_to_name($who) . "', $num) ON DUPLICATE KEY UPDATE points = points + VALUES(points)"
            );
            if (!$silent) {
                $this->bot->send_output(
                    "",
                    "##highlight##$name##end## added ##highlight##$num##end## raidpoints to ##highlight##$who##end##'s account.",
                    "both"
                );
                $this->bot->send_tell(
                    $name, "You added ##highlight##$num##end## raidpoints to ##highlight##$who##end##'s account."
                );
                $this->bot->send_tell(
                    $who, "##highlight##$name##end## added ##highlight##$num##end## raidpoints to your account.($why)"
                );
            }
    ...
    function rem_points($name, $who, $num, $why, $silent = FALSE)
    {
        ...
        if (!$this->bot->core("chat")->get_uid($who)) {
            ...
            if (!$silent) {
                $this->bot->send_output(
                    "",
                    "##highlight##$name##end## removed ##highlight##$num##end## raidpoints from ##highlight##$who##end##'s account.",
                    "both"
                );
                $this->bot->send_tell(
                    $name, "You removed ##highlight##$num##end## raidpoints from ##highlight##$who##end##'s account."
                );
                $this->bot->send_tell(
                    $who,
                    "##highlight##$name##end## removed ##highlight##$num##end## raidpoints from your account. ($why)"
                );
            }
            $this->log($name, $who, "-" . $num, $why);
            return TRUE;
    ...
    */
    ...
    function view_log($name, $timeorname, $time2)
    {
        ...
        if ($ownlogs
            ...
            $logs = $this->bot->db->select(
                "SELECT name, points, by_who, time, why FROM #___raid_points_log WHERE $field " . $value
            ...
            if (!empty($logs)) {
                ...
                foreach ($logs as $log) {
                    ...
                    $inside .= "\n##highlight##" . $log[0] . "##end##: ##$color##" . $log[1]
                        . "##end## points by ##highlight##" . $log[2] . "##end## (" . $log[4] . ")";
                }
