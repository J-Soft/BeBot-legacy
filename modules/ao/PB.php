...
* Revision: $Id: PB.php 1833 2008-11-30 22:09:06Z alreadythere $
*/
...
class PB extends BaseActiveModule
{
    ...
    function __construct(&$bot)
    {
        ...
        $this->help['command']['symb <profession> <slot>']
            = "Shows all known drops of symbs usable by <profession> in <slot>";
        $this->help['notes']
...
$this -> bot -> db -> set_version("symbiants", 2); */
    ...
    function CreateBlob($pb, $symbs)
    {
        ...
        foreach ($symbs as $symb) {
            $title
                = "QL {$symb[0]} {$symb[3]} " . array_search($symb[1], $this->slots) . " symbiant, {$symb[2]} unit aban";
            $title = ucwords($title);
