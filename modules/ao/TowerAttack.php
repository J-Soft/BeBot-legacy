...
class TowerAttack extends BaseActiveModule
{
    function __construct(&$bot)
    {
        ...
        $this->bot->core("settings")
            ->create(
            "TowerAttack", "SpamTo", "both",
            "Where should any tower spam be displayed to? Just gc, just pgmsg, or both?", "gc;pgmsg;both"
        );
        $this->bot->core("settings")
            ->create(
            "TowerAttack", "RelayTowerDamage", "none",
            "Should damage spam of towers be relayed to the private group and/or any linked bots?",
            "none;pgroup;relay;both"
        );
        $this->bot->core("settings")
            ->create(
            "TowerAttack", "ReadOnly", FALSE,
        ...
        $this->bot->core("settings")
            ->create(
            "TowerAttack", "AttackStringUnorged",
        ...
        $this->bot->core("settings")
            ->create(
            "TowerAttack", "AttacksPerBlob", 20, "How many attacks should be shown in the attack history blob?",
            "3;5;8;10;13;15;18;20"
        );
        $this->bot->core("settings")
            ->create(
            "TowerAttack", "VictoryBlobSize", 20, "How many victories from the history should be displayed at once?",
            "5;10;15;20"
        );
        $this->bot->core("settings")
            ->create(
    ...
    function update_table()
    {
        ...
        case 1:
            $this->bot->db->update_table(
                "tower_result", "zone", "add", "ALTER IGNORE TABLE #___tower_result ADD zone VARCHAR(50)"
            );
            $this->bot->db->update_table(
                "tower_result", array(
                    "time",
    ...
    */
    function gmsg($name, $group, $msg)
    {
            ...
            if (preg_match(
                ...
                if (preg_match(
                    ...
                    if (!$this->bot->core("settings")->get("TowerAttack", "ReadOnly")) {
                        $this->bot->db->query(
                            "INSERT INTO #___tower_result (time, win_guild, win_side, lose_guild, "
                                . "lose_side, zone) VALUES ('" . time() . "', '" . mysql_real_escape_string($info[3])
                                . "', '"
                                . $info[2]
                    ...
                    if (preg_match(
                        ...
                        if (preg_match(
                            "/The tower (.+) in (.+) was just reduced to (.+) % health by (.+)!$/i", $msg, $info
                        )
                        ) {
                            ...
                            if (preg_match(
                                "/The tower (.+) in (.+) was just reduced to (.+) % health!$/i", $msg, $info
                            )
                            ) {
        ...
        if ($attack) {
            ...
            if (!($this->bot->core("settings")->get("TowerAttack", "ReadOnly"))
                ...
                $this->bot->db->query(
                    "INSERT INTO #___tower_attack (time, off_guild, off_side, off_player, "
                        . "off_level, off_profession, def_guild, def_side, zone, x_coord, y_coord) VALUES ('"
                        . $infos["time"] . "', '"
                        . mysql_real_escape_string($infos["off_guild"]) . "', '" . $infos["off_side"] . "', '"
                        . $infos["off_player"]
                        . "', '" . $infos["off_level"] . "', '" . $infos["off_profession"] . "', '"
                        . mysql_real_escape_string($infos["def_guild"]) . "', '" . $infos["def_side"] . "', '"
                        . $infos["zone"] . "', '"
    ...
    function format_attack_string($infos, $string, $victory = false)
    {
        if ($victory) {
            ...
            $prtlca
                = "LCA: " . $infos["lca_num"] . "##normal## - " . $lca['name'] . "##end## ##highlight##(L " . $lca['lrng']
                . " - " . $lca['hrng'] . ")##end##\n";

            $battle
                = "##blob_title##" . $infos["zone"] . " (" . $infos["x_coord"] . "x" . $infos["y_coord"] . ")##end##\n";
            $battle .= $prtlca;
            $battle
                .= "Attacker: ##" . $infos["off_side"] . "##" . $infos["off_player"] . " ##end##(" . $infos["off_level"];
            ...
            foreach ($this->suppress as $def_guild => $time) {
                ...
                if (($time + $spamtime) < time()) {
                    ...
                    if (!empty($this->suppressdata[$def_guild])) {
                        ...
                        $inside
                            .= "##" . $this->suppressdata[$def_guild][0]["def_side"] . "##" . $def_guild . " ##end##in ";
                        $inside .= $this->suppressdata[$def_guild][0]["zone"] . " at "
                            . $this->suppressdata[$def_guild][0]["x_coord"];
                        $inside .= "x" . $this->suppressdata[$def_guild][0]["y_coord"] . "##end####blob_text##\n";
                        $lca = $this->get_lcainfo(
                            $this->suppressdata[$def_guild][0]["zone"], $this->suppressdata[$def_guild][0]["x_coord"],
                            $this->suppressdata[$def_guild][0]["y_coord"]
                        );
                        $inside .= "##white##LCA: ##red##x" . $lca['pid'] . "##end####normal## - " . $lca['name']
                            . "##end## ##highlight##(L " . $lca['lrng'] . " - " . $lca['hrng'] . ")##end##\n\n";

                        foreach ($this->suppressdata[$def_guild] as $data) {
                            if (!empty($data["off_guild"])) {
                                $inside
                                    .= "Attacker: ##" . $data["off_side"] . "##" . $data["off_player"] . "##end## (##"
                                    . $data["off_side"] . "##" . $data["off_guild"] . "##end##, level "
                                    . $data["off_level"] . " " . $data["off_profession"] . ")\n";
                            }
                            else {
                                $inside
                                    .= "Attacker: ##" . $data["off_side"] . "##" . $data["off_player"] . "##end## (Level "
                                    . $data["off_level"] . " " . $data["off_profession"] . ")\n";
                            }
                        }

                        $msg = "Suppressed Attacks on ##" . $this->suppressdata[$def_guild][0]["def_side"] . "##"
                            . $def_guild . "##end## :: " . $this->bot
                            ->core("tools")
