...
class Symb_sql extends BaseActiveModule
{
    ...
    function symb($args)
    {
        ...
        $args = preg_replace(explode(',', '/\b' . $this->bot
            ->core('professions')->get_shortcuts('\b/,/\b') . '\b/'), $this->bot
        ...
        foreach ($this->bot->core('professions')->get_profession_array() as
                 $profession)
        {
            $args = str_replace($profession, $this->bot->core('professions')
        ...
        foreach ($args as $arg)
        {
            ...
            if (in_array($arg, $this->bot->core('professions')
                ->get_unit_array())
            ) {
