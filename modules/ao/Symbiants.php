...
class Symb_sql extends BaseActiveModule
{
    ...
    function __construct(&$bot)
    {
        ...
        $this->help['command']['symb <level>[-<level2>]']
            = "Find symbs of QL level. If [-<level2>] is specified it finds symbs between the two";
    ...
    function symb($args)
    {
        ...
        if (empty($symbiants)) {
            ...
            foreach ($symbiants as $symb) {
                $title = "QL {$symb['ql']} {$symb['symb']} " . array_search($symb['slot'], $this->slots)
