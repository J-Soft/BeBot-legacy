    ...
    */
    function __construct(&$bot)
    {
        ...
        $this->bot->core("settings")
            ->create(
            "AlienAttack", "Spam", "none", "Should the bot spam to gc or tells (on logon) or both?", "none;gc;tell;both"
        );
        $this->bot->core("settings")
            ->create(
            "AlienAttack", "Channel", "gc",
            "Into which channel should any output about alien attacks and city changes be send?", "gc;pgmsg;both"
        );
        $this->bot->core("settings")
            ->create(
            "AlienAttack", "PublicTimer", false,
            "Should a public timer in addition to the periodic spam be created on cloak up and down?"
        );
        $this->bot->core("settings")
            ->create(
            "AlienAttack", "CloakReminder", TRUE,
    ...
    function timer($name, $prefix, $suffix, $delay)
    {
        ...
        if ($type == "spam") {
            $this->bot->send_output(
                "", "##red##Warning##end##: Alien Raid in City is in Progress. Please dont enter the city", $channel
            );
            if ($delay == 0) {
                $this->spam = FALSE;
            }
        ...
        elseif ($type == "cloak") {
            if ($delay == 0) {
                $this->bot->send_output(
                    "", "Cloaking device was disabled one hour ago. It is now possible to enable it again.", $channel
                );
            }
            elseif ($delay != 3600) {
                $this->bot->send_output(
                    "", "Cloaking device is disabled. It will be possible to enable it again " . $suffix, $channel
                );
            }
            if ($delay == 60) {
        ...
        elseif ($type == "cloakready") {
            if ($delay == 0) {
                $this->bot->send_output(
                    "", "Cloaking device has been enabled one hour ago. Alien attacks can now be initiated.", $channel
                );
            }
        }
    }
    ...
    */
    function city_blob()
    {
        ...
        if (!$result) {
            ...
            $state .= $ttchange ?
                " It will be possible to ##highlight##" . $status2 . "##end## it in ##highlight##" . $avilmin
                    . "##end## minutes and ##highlight##" . $avilsec . "##end## seconds. " : "";
    ...
    */
    function gmsg($name, $group, $msg)
    {
        if ($name == "0") {
            ...
            $player = "";
            if (preg_match(
                "/Your radar station is picking up alien activity in the area surrounding your city./i", $msg
            )
            ) {
                ...
                if (preg_match("/Your city in (.+) has been targeted by hostile forces./i", $msg, $info)) {
                    ...
                    $zone = $info[1];
                    $this->bot->send_output(
                        "", "Our city in " . $zone . " is about to be under attack! 0MGZ RUN!!!!", $channel
                    );
                }
                else {
                    if (preg_match("/(.+) turned the cloaking device in your city off./i", $msg, $info)) {
                        ...
                        if ($this->bot->core("settings")
                            ...
                            $this->bot->core("timer")
                                ->add_timer(
                                false, $player, 60 * 60 + 1,
                                $this->bot->core("shortcuts")
                        ...
                        $this->bot->send_output(
                            "", "##highlight##" . $player
                            . "##end## turned the cloaking device in our city ##highlight##off##end##!", $channel
                        );
                    }
                    else {
                        if (preg_match("/(.+) turned the cloaking device in your city on./i", $msg, $info)) {
                            ...
                            $this->bot->core("timer")
                                ->add_timer(
                                false, "city", 60 * 60 + 1, $player . " cloakready", "internal", 0, "CityCloakReady"
                            );
                            ...
                            $this->bot->send_output(
                                "", "##highlight##" . $player
                                . "##end## turned the cloaking device in our city back ##highlight##on##end##!",
                                $channel
                            );
                        }
                        else {
                            if (preg_match(
                                "/(.+) initiated removal of the organization headquarters in (.+)/i", $msg, $info
                            )
                            ) {
                                $action = "HQ";
                                $player = $info[1];
                                ...
                                if (preg_match("/(.+) removed the organization headquarters in (.+)/i", $msg, $info)) {
                                    ...
                                    $this->bot->send_output(
                                        "",
                                        "##highlight##" . $player . "##end## has removed our HQ in ##highlight##"
                                            . $zone
                                    ...
                                    if (preg_match("/(.+) initiated removal of a (.+) in (.+)/i", $msg, $info)) {
                                        ...
                                        $this->bot->send_output(
                                            "",
                                            "##highlight##" . $player . "##end## is removing a " . $info[2]
                                                . " at our city in ##highlight##"
                                                . $zone . "##end##.", $channel
                                        );
                                    }
                                    else {
                                        if (preg_match("/(.+) removed a (.+) in (.+)/i", $msg, $info)) {
                                            ...
                                            $this->bot->send_output(
                                                "",
                                                "##highlight##" . $player . "##end## removed a " . $info[2]
                                                    . " at our city in ##highlight##"
                                                    . $zone . "##end##.", $channel
                                            );
                                        }
                                            ...
                                            if (preg_match(
                                                "/^The upkeep for your organization housing has not been paid./i", $msg
                                            )
                                            ) {
                                                $action = "payment";
                                                $player = "";
    ...
    function notify($name)
    {
        ...
        if ($this->spam && ($setting == "tell" || $setting == "both")) {
            $this->bot->send_tell(
                $name, "##red##Warning##end##: Alien Raid in City is in Progress. Please dont enter the city"
            );
        }
    }
}
