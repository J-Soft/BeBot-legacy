    ...
    function show_lc($iarea = NULL, $lrange = 0, $hrange = 300)
    {
        if ($iarea == "--all--") {
            ...
            if (!$iarea) {
                ...
                $areas = $this->bot->db->select(
                    "select distinct(area),count(area) from #___land_control_zones where area like '%" . $iarea
                        . "%' group by area"
                );
            }
            if (!empty($areas)) {
                foreach ($areas as $area) {
                    ...
                    $temp
                        .= "<div align=center><u><font color=#10a5e5>" . $area[0] . " (" . $area[1] . ")</font></u></div>";
                    if ($lrange == $hrange) {
                        $lcs = $this->bot->db->select(
                            "select id, lrange, hrange, area, huge, x, y, name from #___land_control_zones where area='"
                                . $area[0] . "' AND lrange<=" . $lrange . " AND hrange>=" . $hrange . " order by huge"
                        );
                    }
                    else {
                        $lcs = $this->bot->db->select(
                    ...
                    if (!empty($lcs)) {
                        foreach ($lcs as $lc) {
                            $temp
                                .= " Area: " . $lc[7] . "<br> Range: " . $this->conv($lc[1]) . "-" . $this->conv($lc[2])
                                . "<br> Coords: " . $this->coords($lc[5]) . "x" . $this->coords($lc[6])
                                . "<br> Hugemap: " . $lc[4] . "<br><br>";
