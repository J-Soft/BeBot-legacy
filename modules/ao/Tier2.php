...
class Tier2 extends BaseActiveModule
{
    ...
    */
    ...
    function join_window($name)
    {
        ...
        $blob .= "[" . $this->bot->core("tools")
            ->chatcmd("t2 join all", "Join All Raffles") . "]  ";

        if ($this->bot->core("security")->check_access($name, "leader")) {
            $blob .= "[" . $this->bot->core("tools")
                ->chatcmd("t2 admin", "Administration Console") . "]  ";
        }

        $blob .= "[" . $this->bot->core("tools")
            ->chatcmd("t2 join", "Refresh") . "]\n\n";
    ...
    function admin_console($name)
    {
        ...
        $blob .= "  -  [" . $this->bot->core("tools")
            ->chatcmd("t2 admin", "Refresh") . "]";
        $blob .= "  -  [" . $this->bot->core("tools")
            ->chatcmd("t2 join", "Your Status") . "]\n\n";

        $blob .= "[" . $this->bot->core("tools")
            ->chatcmd("t2 admin loot", "Small Loot Window") . "]  -  ";
        $blob .= "[" . $this->bot->core("tools")
            ->chatcmd("t2 admin user", "User Admin") . "]\n\n";
    ...
    function admin_user_console($name)
    {
        ...
        $blob .= "[" . $this->bot->core("tools")
            ->chatcmd("t2 admin", "Administration Console") . "]  [" . $this->bot
            ->core("tools")->chatcmd("t2 admin user", "Refresh") . "]\n\n";
        ...
        foreach ($this->users as $u_name => $data) {
            ...
            foreach (array("embryo",
                ...
                if ($data[$cat] && $data["got_" . $cat]) {
                    $blob .= "  [" . $this->bot->core("tools")
                        ->chatcmd("t2 admin rejoin $u_name $cat", "Re-Add to Raffle List") . "]  ";
                }
                else if ($data[$cat]) {
                    $blob .= "  [In list]";
                }
    ...
    function make_ref($item)
    {
        ...
        return $this->bot->core("tools")
            ->make_item($this->ids[$item], $this->ids[$item], $this->qls[$item], $this->names[$item]);
    }
