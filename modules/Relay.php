...
class Relay extends BaseActiveModule
{
    function __construct(&$bot)
    {
        ...
        $this->bot->core('prefs')
            ->create(
            'AutoInv', 'recieve_auto_invite', 'Automatic invites to private group should be?', 'Off', 'Off;On'
        );
        ...
        $this->bot->core("settings")
            ->create(
            'Relay', 'Inc', 'Both', 'Where should incoming messages relay to', 'Both;Guildchat;Privgroup;None'
        );
        $this->bot->core("settings")
    ...
    */
    function tell($name, $msg)
    {
        ...
        elseif (strtolower($input['com']) == 'gcr' && $this->bot->core("settings")->get('Relay', 'Status')
            && (($this->bot->core("settings")
    ...
    function relay_to_bot($msg, $chat = true, $alt = FALSE, $type = "notchat")
    {
        ...
        if ($this->bot->core("settings")->get('Relay', 'Status')
            ...
            elseif (strtolower(
                ...
                $this->bot->db->query(
                    "INSERT INTO #___relay (time, type, botname, msg) VALUES (" . time() . ", '$prefix', '"
                        . $this->bot->botname . "', '" . mysql_real_escape_string($msg) . "')"
                );
            }
        }
    }
    ...
    */
    ...
    function cron($cron)
    {
        if ($cron == 300) {
            if (!$this->guildnameset) {
                ...
                $this->bot->core("settings")
                    ->create(
                    'Relay', 'Pgname', $this->bot->guildname . ' Guest',
                    'What name should we show when we relay from the private group?'
                );
                $this->bot->core("settings")
                    ->create(
                    'Relay', 'Gcname', $this->bot->guildname, 'What name should we show when we relay from guild chat?'
                );
                ...
                if (!empty($result)) {
                    foreach ($result as $res) {
                        ...
                        elseif (strtolower($res[1]) == "gcr" && $this->db_relay) {
                            if (ucfirst(strtolower($res[2])) != $this->bot->botname
    ...
    function extpgjoin($pgname, $name, $cron = FALSE)
    {
        ...
        if ($this->bot->core("settings")->get('Relay', 'Status')
            ...
            $online = $this->bot->db->select(
                "SELECT nickname, status_gc, status_pg, level FROM #___online WHERE (status_gc = 1 OR status_pg = 1) AND botname = '"
                    . $this->bot->botname . "' ORDER BY nickname"
            );
            if (empty($online)) {
                $online = "";
            }
    ...
    function inc_com($name, $msg, $source)
    {
        ...
        case 'buddy':
            ...
            if ($column) {
                $sql
                    = "INSERT INTO #___online (nickname, botname, " . $column . ", " . $column . "_changetime" . $leveln
                    . ") ";
                $sql .= "VALUES ('" . $nickname . "', '" . $name . "', '" . $newstatus . "', '" . time() . "'" . $level
                    . ") ";
                $sql .= "ON DUPLICATE KEY UPDATE " . $column . " = '" . $newstatus . "', " . $column . "_changetime = '"
                    . time() . "'" . $levele;
                $this->bot->db->query($sql);
            }
            Break;
