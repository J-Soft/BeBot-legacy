...
class Mail extends BaseActiveModule
{
    function __construct(&$bot)
    {
        ...
        $this->bot->core("settings")
            ->create(
            $this->module_name, "Max_life_read", "6_months", "How long should a read message be kept?",
            "1_week;2_weeks;1_month;6_months;1_year;2_years"
        );
        $this->bot->core("settings")
            ->create(
            $this->module_name, "Max_life_unread", "1_year", "How long should an unread message be kept?",
            "1_week;2_weeks;1_month;6_months;1_year;2_years"
        );
        ...
        $this->bot->core("prefs")
            ->create(
            $this->module_name, "Life_read", "How long should a read message be kept?", "1_month",
            "1_week;2_weeks;1_month;6_months;1_year;2_years", "GUEST"
        );
        $this->bot->core("prefs")
            ->create(
            $this->module_name, "Life_unread", "How long should an unread message be kept?", "6_months",
            "1_week;2_weeks;1_month;6_months;1_year;2_years", "GUEST"
        );
        $this->bot->core("prefs")
            ->create(
            $this->module_name, "Logon_notification", "Do you want to be notified about new mail when you log on?",
            "Yes", "Yes;No", "GUEST"
        );
    ...
    function notify($name, $startup = false)
    {
        ...
        if ((!$startup)
            ...
            if ($no_of_messages != 0) {
                $this->bot->send_tell(
                    $name, $this->make_item_blob(
                        "You've got ##error##$no_of_messages##end## new messages.", $this->mail_list($name)
                    )
                );
            }
        }
    ...
    function mail_read($user, $id)
    {
        ...
        if (!empty($messages)) {
            ...
            $window .= "[" . $this->bot->core("tools")
                ->chatcmd(
                "mail send {$message['sender']} The message you sent on {$message['recieved']} has been read",
                "Notify sender"
            ) . "]";
            $time = strtotime(
                "+" . str_replace(
            ...
            $query
                = "UPDATE #___mail_message SET is_read=true, expires=FROM_UNIXTIME('$time') WHERE id=$id AND is_read=false";
        ...
        elseif (empty($message)) {
            ...
            if (!empty($online)) {
                foreach ($online as $send) {
                    $this->bot->send_tell(
                        $send, $this->make_item_blob("You've just Recieved a new message.", $this->mail_list($send))
                    );
                }
            }
