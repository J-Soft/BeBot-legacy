    ...
    */
    function __construct(&$bot)
    {
        ...
        $this->bot->core("settings")
            ->create("Alts", "Security", TRUE, "Should security restrictions be enabled to prevent users from gaining higher access levels by adding alts with higher access level when Usealts for the security module is enabled?");
    }
    ...
    function command_handler($name, $msg, $origin)
    {
        ...
        if (($this->bot->core("settings")
                 ->get("Alts", "Security") == TRUE) && ($this->bot
                                                            ->core("settings")
    ...
    */
    function add_alt($name, $alt, $admin = 0)
    {
        ...
        if (($this->bot->core("settings")
                 ->get("Alts", "Security") == TRUE) && ($this->bot
                                                            ->core("settings")
                                                            ->get("Security", "UseAlts") == TRUE) && ($admin == 0)
        ) {
            $security = TRUE;
        }
        ...
        if ($this->bot->core("settings")
            ...
            $this->bot->send_tell($alt, "Alt Confirmation :: " . $this->bot
