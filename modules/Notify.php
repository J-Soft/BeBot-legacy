...
class Notify extends BaseActiveModule
{
    function __construct(&$bot)
    {
        ...
        $this->help['command']['notify cache update']
            = "Updates the notify cache with the latest players on the notify list.";
    }
    ...
    function command_handler($name, $msg, $origin)
    {
        ...
        Default:
            if (strtolower($com['arg']) == "on" || strtolower($com['arg']) == "off"
    ...
    function show_notify_list()
    {
        ...
        return $total . " Characters on notify: " . $this->bot->core("tools")
            ->make_blob($membercount . " Member", $member) . ", "
            . $this->bot->core("tools")
                ->make_blob($guestcount . " Guests", $guest) . ", "
            . $this->bot->core("tools")
                ->make_blob($othercount . " Others", $other);
    }
