...
class Vote extends BaseActiveModule
{
    function __construct(&$bot)
    {
        ...
        $this->bot->core("settings")
            ->create(
            'Vote', 'New', 'ADMIN', 'Who should be able to start new votes',
            'GUEST;MEMBER;LEADER;ADMIN;SUPERADMIN;OWNER'
        );
        $this->bot->core("settings")
            ->create(
            'Vote', 'End', 'ADMIN', 'Who should be able to end votes', 'GUEST;MEMBER;LEADER;ADMIN;SUPERADMIN;OWNER'
        );
        $this->bot->core("settings")
        ...
        $this->help['command']['vote restrict <vote#> [min_level]']
            = "Only players with <min_level> access can vote on vote <vote#>. If min_level is omitted a menu of availible levels is shown";
        $this->help['command']['vote addopt <vote#> <option1> [;option 2;...;option n]']
            = "Adds adds new option(s) to <vote#>";
        $this->help['command']['vote delopt <option_id>'] = "Remove option with id <option_id>";
        $this->help['command']['vote edit <option_id> <newtext>']
            = "Replaces the text for option with id <option_id> with <newtext>";
        ...
        $this->help['notes']
            .= "The time argument needs to be a format as described by http://www.gnu.org/software/tar/manual/html_node/tar_109.html ";
        $this->help['notes']
            .= "This means that formats like most dates and also '1 month', 'next thursday' etc. are allowed.<br>";
        $this->help['notes']
            .= "Since semicolon (;) is used as a separator it cannot be used in vote- or option descriptions.";
    }
    ...
    function end_vote($name, $vote_id)
    {
        ...
        if ($vote = $this->bot->db->select($query)) {
            ...
            if (($name !== $vote['vote_starter'])
                && (!$this->bot->core("security")->check_access(
                    $name, $this->bot
                        ->core("settings")->get('Vote', 'End')
                ))
            ...
            $query
                = "SELECT COUNT(id) AS votes, option_id FROM #___vote_ballots WHERE vote_id = $vote_id GROUP BY option_id ORDER BY votes DESC";
            ...
            foreach ($vote_counts as $vote_count) {
                $query
                    = "UPDATE #___vote_options SET votes = {$vote_count['votes']} WHERE id = {$vote_count['option_id']}";
                $this->bot->db->query($query);
            }
            ...
            $query
                = "SELECT votes, description FROM #___vote_options WHERE vote_id = $vote_id ORDER BY votes DESC LIMIT 1";
            ...
            $query
                = "UPDATE #___votes SET winner = '{$winner['description']}', total_votes = $total, votes_percent = $percent, end_time=-1 WHERE id = $vote_id";
            $this->bot->db->query($query);
        }
    ...
    function vote_interface($name, $vote = false)
    {
        if ($vote === false) {
            ...
            if (!empty($votes)) {
                ...
                return $this->bot->core("tools")
                    ->make_blob("Availible votes", $window);
            }
            else {
                return ("votes = Empty");
            }
        ...
        elseif ($vote == 'all') {
            ...
            if (true) {
                ...
                $query
                    = " SELECT id, description FROM #___votes WHERE id=$vote UNION SELECT id, description FROM #___vote_options WHERE vote_id=$vote";
